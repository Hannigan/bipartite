...
def create_regular_nonmultigraph(N,d,doPrint = False):
    # create a list with d of each node on the right side
    right_sides_dict = {} 
    for left in range(N):
        right_sides_dict[left] = []
    for right in range(N,2*N):
        if doPrint: print "right_sides_dict:" + str(right_sides_dict)
        keys = sample([k for k in right_sides_dict.keys() if len(right_sides_dict[k]) < d], d) 
        if doPrint: print "keys: " + str(keys)
        for i in keys:
            right_sides_dict[i].append(right)
        
    right_sides = []
    for k in right_sides_dict.keys():
        right_sides += right_sides_dict[k] 
